...
Write a function called addTwo that takes in an array and adds two to every value 
using a for loop. Place the new value in a new array. Return the new array.
------------------------------------------------------------------------------------------------ */
...
Write a function named typeNum that, given an array as input, uses filter 
to return an array containing only the numbers.
...
uses filter to return an array containing only strings that contain 
...
const containsAnd = (arr) => {
  const filtered = arr.filter(string => string.includes('and'));
  return filtered;
};
...
Write a function named oddValues that, given an array of integers as input, uses filter 
to return an array containing only the odd integers.
...
const notInFirstArray = (forbiddenValues, arr) => {
  const filtered = arr.filter(number => !forbiddenValues.includes(number));
  return filtered;
};
...
For example, getBaseStatGreaterThan(snorlaxData.stats, 50) will return an 
array containing the 'special-defense' and 'special-attack' objects.
------------------------------------------------------------------------------------------------ */
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const filtered = arr.filter(n => n.baseStat > minBaseStat);
  return filtered;
};
...
getBaseStatGreaterThan function from challenge 7. For this function, 
extend your solution from challenge 7 to only return the name of the stat, 
...
const getStatName = (arr, minBaseStat) => {
  const filtered = arr.filter(n => n.baseStat > minBaseStat);
  const newArray = filtered.filter(n => n.stat.name);
